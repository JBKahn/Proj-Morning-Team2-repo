...
class EventAPIView(APIView):
    ...
    def get(self, request, format='JSON', *args, **kwargs):
        ...
        start = dt.datetime(2014, 11, 7, 12, 0)
        end = dt.datetime(2014, 11, 7, 13, 0)
        recur_until = dt.datetime(2014, 11, 17, 13, 0)
        start = local.localize(start)
        end = local.localize(end)
        recur_until = local.localize(recur_until)


        event = ApiInterface.create_google_json(title="fweuifhqewfiu", start=start, end=end, all_day=False, description=None, location=None)
        ...
        return Response(ApiInterface.post_event_to_calendar(user=request.user, calendar_id=calendar_id, event=event, tag="ASSIGNMENT", org="dsdfs"))
